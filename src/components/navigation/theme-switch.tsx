...
export const ThemeSwitch = ({ className }: { className?: string }) => {
  ...
  useEffect(() => {
    ...
    function onChange(e: Event | StorageEvent) {
      ...
      if (t) setTheme(t);
        ...
        setTheme(
          (process.env.NEXT_PUBLIC_DEFAULT_THEME as ThemeType) ?? "light",
        );
    }
  ...
  const toggleTheme = () => {
    ...
    if (typeof window !== "undefined" && window.__setTheme) {
      ...
      try {
        ...
        console.error(`Error writing localStorage on toggleTheme: ${e}`);
      }

      // biome-ignore lint/suspicious/noDocumentCookie: cookie override exception
      document.cookie = `theme=${encodeURIComponent(next)}; path=/; Max-Age=${
        60 * 60 * 24 * 365
      }`;
  ...
  const checked = mounted ? theme === "dark" : false;

  return (
    <label className={cn(className, "theme-switch")}>
      <input
        type="checkbox"
