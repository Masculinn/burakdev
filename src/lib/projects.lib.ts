...
export default function SaaS() {
  return (
    <>
      <Head>
        <title>Melancholy Moments - Reflections</title>
        <meta
          name="description"
      ...
      <nav className="container flex flex-row  items-center justify-between max-w-7xl mx-auto py-8 text-xl text-stone-800 fixed top-0 right-1/2 translate-x-1/2 z-[999]">
      ...
      <MotionImage
        ...
        imageUrl={"/assets/projects/saas-background.webp"}
        pieces={121}
            ...
            children={Array.from({ length: 4 }).map((_, i) => (
              <Fragment key={i}>
                <MotionImage
                  ...
                  transition="delayedSmooth"
                  configView={{ once: false, amount: "some" }}
                  imageUrl={featureList2[i].src}
                  pieces={25}
                  wrapperClassName="rounded-md aspect-video mb-4"
                  animations={["fadeIn", "burakPulse"]}
                />
        ...
        <div className="max-w-3xl mx-auto lg:-mt-32 -mt-48 ">
          ...
          <div className="pt-4 flex justify-center items-center flex-row gap-2">
            <Button
              ...
              onClick={() => setReverseAnimation((prev) => !prev)}
            >
              <span className="mr-2">Show Code</span>
              <Code />
            </Button>
...
`,
    desc: "Crypto Landing App",
  },
  agency: {
    code: `import { Code, MousePointerClick } from "lucide-react";
import MotionImage from "@/components/MotionProvider/motion-image";
import MotionQueue from "@/components/MotionProvider/motion-queue";
import { Skeleton } from "@/components/ui/skeleton";
import { Badge } from "@/components/ui/badge";
...
const heroContent = {
  ...
  description: "A collection of 10,000 worldly Koalas each with their unique skillsets. Their mission is to protect the world from evil.",
};

export default function Agency() {
  return (
    <div className="relative bg-[#060707] overflow-hidden text-white min-h-screen">
      <nav className="relative z-50 flex items-center justify-between px-8 py-5 ">
        <MotionContainer
          ...
          delay={0.5}
          duration={0.6}
          className="text-4xl font-bold text-[#02C173]"
        >
          Koala.
        </MotionContainer>
        <div className="flex flex-row items-center justify-center lg:gap-3 gap-2 lg:scale-100 scale-90">
          <Link
            href="/"
            className="flex hover:text-white"
            rel="noopener noreferrer"
      ...
      <div className="relative flex flex-col items-center justify-center px-8 py-10 min-h-[70vh] z-30 ">
        <MotionContainer
          mode={["fadeIn", "bounceX"]}
          elementType="div"
          duration={4}
          className="absolute bg-[#15d98bfd] h-[362px] w-[362px] rounded-full blur-[120px] -top-32 -left-32"
        />
        <div className="relative z-40 max-w-xl text-center flex items-center justify-center flex-col gap-4 ">
          <MotionQueue
            elementType="h1"
            animations={Array(3).fill({
              mode: ["fadeUp", "blurIn"],
              duration: 0.8,
              transition: "cubicElastic",
            })}
            delayLogic="sinusoidal"
            className="font-extrabold lg:text-6xl text-5xl uppercase tracking-tighter"
          >
            {heroContent.title.map((word, i) => (
              <span
                key={word}
                className={i === 1 ? "text-[#02C173] mx-2" : "mx-2"}
              >
                {word}
              </span>
            ))}
          ...
          <MotionContainer
            ...
            duration={1.2}
            delay={0.8}
            className="tracking-tighter text-lg mb-10 leading-relaxed"
          >
            {heroContent.description}
          </MotionContainer>
          <div className="flex lg:flex-row flex-col items-center justify-center gap-2">
            <Button className="bg-[#02C173] px-8 py-3 rounded-lg flex items-center hover:scale-105 transition-transform w-full">
              <Code className="mr-2" />
              <span> Code</span>
            </Button>
            <Button className="border-2 border-[#02C173] px-8 py-3 rounded-lg flex items-center hover:bg-[#02C173]/20 transition-colors">
        ...
        <div className="hidden md:block py-5 animate-spin">
          <svg
            width="108"
            height="108"
            viewBox="0 0 138 138"
      ...
      <div className="absolute right-0 bottom-0 z-20">
        <MotionImage
          ...
          delayLogic="quantum"
          transition="cubicSmooth"
          imageUrl="https://res.cloudinary.com/ddcg0rzlo/image/upload/v1651418249/new-nft_tlfisy.png"
          pieces={144}
          wrapperClassName="w-80 h-80 lg:w-96 lg:h-96 object-cover -rotate-45 lg:translate-y-1/3 translate-x-1/3"
          animations={["wave", "fadeIn"]}
        />
      </div>
    </div>
  );
...
const desc = "Explore a curated collection of unique digital assets and stunning NFT art. Embrace the future of digital creativity with style and elegance.".trim().split(/\s+/);
...
export default function Nft() {
  ...
  return (
    <>
      <Head>
        <title>Modern NFT Landing Page</title>
        <meta
          name="description"
